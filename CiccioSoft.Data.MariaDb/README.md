...
ADO.NET provider for MariaDB Embedded, built on top of the CiccioSoft.Interop.MariaDb layer.
...
- Uses the CiccioSoft.Interop.MariaDb layer for native operations
...
This provider is built on top of `CiccioSoft.Interop.MariaDb`, which handles the low-level P/Invoke interactions with MariaDB Connector/C. The provider translates .NET ADO.NET calls into native MariaDB operations.
...
```csharp
using var connection = new MariaDbConnection("Server=localhost;Database=mydb;User=root;Password=secret;");
